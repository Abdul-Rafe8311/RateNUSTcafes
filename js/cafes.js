...
function renderStars(rating, size = 14) {
  ...
  let h = '';
  for (let i = 0; i < full; i++)  h += `<svg style="${s}fill:#C8975A;" viewBox="0 0 24 24"><path d="${starPath}"/></svg>`;
  if (half)                        h += `<svg style="${s}fill:none;stroke:#C8975A;stroke-width:1.5" viewBox="0 0 24 24"><path d="${starPath}"/></svg>`;
  for (let i = 0; i < empty; i++) h += `<svg style="${s}fill:none;stroke:rgba(200,151,90,0.25);stroke-width:1.5" viewBox="0 0 24 24"><path d="${starPath}"/></svg>`;
  return h;
}
